...
module.exports = async ({ getNamedAccounts, deployments, getChainId }) => {
    ...
    if (chainId != 31337) {
        ...
        // First we setup a listener and then we call the create tx
        ...
        const listenForEvent = new Promise(async (resolve, reject) => {
            RandomSVG.once("CreatedUnfinishedRandomSVG", async () => {
                ...
                await tx.wait(1);
                log(
                    `You can view the tokenURI here ${await RandomSVG.tokenURI(0)}`
                );
                resolve();
            });
        });
        const result = Promise.race([timeour, listenForEvent]);
        ...
        await resourceLimits;
    } else {
        tx = await RandomSVG.create({ gasLimit: 300000 });
        let receipt = await tx.wait(1);
        tokenId = receipt.events[3].topics[2];
        const VRFCoordinatorMock = await deployments.get("VRFCoodinatorMock");
