...
import "@chainlink/contracts/src/v.08/VRFConsumerBase.sol";
...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase, Ownable {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber) internal override {
        ...
        emit CreatedUnfinishedRandomSVG(token, randomNumber);
    }
    ...
    function generateSVG(uint256 _randomness) public view returns (string memory finalSvg) {
        ...
        finalSvg = string(abi.encodePacked("<svg xmlns='https://www.w3.org/2000/svg' height='", uint2str(size), "' width='", uint2str(size), "'>"));
